...
class InputValidatorTest : ShouldSpec({
    ...
    context("시도 횟수가") {
        context("숫자가 아니라면") {
            ...
            should("예외가 발생한다.") {
                inputs.forAll {
                    val exception = shouldThrow<IllegalArgumentException> { InputValidator.validTryCount(it) }
        ...
        context("0 미만이라면") {
            ...
            should("예외가 발생한다.") {
                val exception = shouldThrow<IllegalArgumentException> { InputValidator.validTryCount(input) }
